...
Database.prototype = {
    ...
    execute: function(statement) {
        return new Promise((resolve, reject) => { //usado arrow function por causa do this
            setTimeout(() => {
                const parsedStatement = this.parseStatement(statement);
                if (parsedStatement) {
                    try {
                        resolve(this[parsedStatement.command](parsedStatement.data));
                    } catch (e) {
                        reject(new DatabaseError(statement, e));
                    }
                } else {
                    reject(new InvalidDatabaseStatement(statement));
                }
            }, 1000);
        });
    }
...
database.execute("create   table author(id number, name string,   age   number, city  string,state string, country string  ) ").then(function() {
    ...
    return database.execute("select  id, name , age from  author ").then(function(result) {
        console.log(result);
    });
}).catch(function(e) {
